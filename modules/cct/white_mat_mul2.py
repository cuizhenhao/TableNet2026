        ...
        for i in range(self.in_features):
            for j in range(self.out_features):
                ...
                for k in range(self.mid_features):
                    mul_results.append(mul_table[k][left_input[:, i, k], right_input[:, k, j]])
                for k in range(self.mid_features):
                    out[:, i, j] = add_tables[k][out[:, i, j], mul_results[k]]
